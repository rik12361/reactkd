...
const DisneyCmsDetailWindow = ( { id, showOrHideCms, mutationType, setRefreshCms } ) => {
    ...
    const handleSubmit = async (e) => {
        ...
        var mutCms = { id, email, url, description, markedAsPrivate, updates, updated, created};
        var sMethode = mutationType === "create" ? "POST" : "PUT";
        var sUrl = 'http://localhost:8080/disneycmsreport' ;
    ...
    const getCms = async () => {
        const rawResponse = await fetch('http://localhost:8080/disneycmsreport/' + id,
        {
            headers: {
                'content-type' : 'application/json'
            },
...
export default DisneyCmsDetailWindow